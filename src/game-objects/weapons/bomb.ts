...
/**
 * For the bomb weapon, this is a little bit different from our existing weapons in that
 * the player can spawn multiple bomb instances at one time (depending on their inventory level),
 * up to a max number on the screen.
 *
 * Each bomb instance, will have its own state like: "idle -> flashing -> explode". In the idle state,
 * there is no animation for the bomb, and we countdown until we enter the next state. In the flashing
 * state, the bomb animates between a few colors and we do another countdown until we enter the next
 * state. Finally, in the explode state, we hide the game object, and show the explosion animation.
 *
 * In the last state, we would do collision detection to see if we hit any enemies/player. After the animation,
 * we hide and make object inactive, that way we can re-use those game objects.
 */
